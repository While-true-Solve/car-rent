...
import type { ClassRepository } from 'src/core/repository/class.repository';
...
@Injectable()
export class ClassCarService extends BaseService<CreateClassCarDto, UpdateClassCarDto, ClassCars> {
  constructor(
    @InjectRepository(ClassCars) private readonly classCarRepo: ClassCarRepository,
  ...
  async createClassCar(createClassCarDto: CreateClassCarDto) {
    const classs = await this.classRepo.findOne({ where: { id: createClassCarDto.class_id }})
    if(!classs){
      throw new NotFoundException('Classs not found')
    }

    const car = await this.carRepo.findOne({ where: { id: createClassCarDto.car_id } })
    if (!car) {
  ...
  async updateClassCar(id: string, updateClassCarDto: UpdateClassCarDto) {
    const classCar = await this.classCarRepo.findOne({ where: { id }, relations: ['classEntity', 'car'] })
    if (!classCar) {
    ...
    if(class_id){
      const classId = await this.classRepo.findOne({ where: { id: updateClassCarDto.class_id }})
      if(!classId){
    ...
    if (car_id) {
      const carId = await this.carRepo.findOne({ where: { id: updateClassCarDto.car_id } })
      if (!carId) {
        throw new NotFoundException('Car not found')
      }
      classCar.car = carId;
  ...
  async removeClassCar(id: string) {
    const classCar = await this.classCarRepo.findOne({where: { id }})
    if(!classCar){
      throw new NotFoundException('ClassCar not found')
    }

    await this.classCarRepo.remove(classCar)
