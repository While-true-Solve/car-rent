...
import { AdoptedCar, Car, Customer, type adopted_carRepository, type carRepository, type CustomerRepository } from 'src/core';
import { successRes } from 'src/infrastructure/response/successRes';

@Injectable()
export class AdopdetCarService extends BaseService<CreateAdopdetCarDto, UpdateAdopdetCarDto, AdoptedCar> {
  constructor(
    @InjectRepository(AdoptedCar) private readonly adopdet_carRepo: adopted_carRepository,
  ...
  async createAdoped(createAdopdetCarDto: CreateAdopdetCarDto) {
    ...
    const car = await this.carRepo.findOne({ where: { id: createAdopdetCarDto.car_id } })
    if (!car) {
  ...
  // async findAllAdopdet() {
  //   const adopdet_car = await this.adopdet_carRepo.find({ relations: ['customer', 'car'] });
  //   return successRes(adopdet_car)
  // }


  // async findOne(id: string) {
  //   const adopdet = await this.adopdet_carRepo.findOne({ where: { id }, relations: ['customer', 'car'] })
  ...
  async updateAdopdet_car(id: string, updateAdopdetCarDto: UpdateAdopdetCarDto) {
    ...
    if (updateAdopdetCarDto.adopted_date && updateAdopdetCarDto.adopted_date > today) {
      throw new BadRequestException('Adopdet data cannot be in the future')
    }


    if (updateAdopdetCarDto.customer_id) {
      const customer = await this.customerRepo.findOne({ where: { id: updateAdopdetCarDto.customer_id } })
      if (!customer) {
        throw new NotFoundException('Customer not found')
      }

      // Shu customer id  boshqa joyda bondmi yoki yoqmi tekshiradi 
      const existsAdopdet = await this.adopdet_carRepo.findOne({ where: { customer: { id: updateAdopdetCarDto.customer_id } } })

      if (existsAdopdet && existsAdopdet.id !== adopdet.id) {
        throw new BadRequestException('This customer has already adopted a car')
      }
    ...
    if (updateAdopdetCarDto.car_id) {
      const car = await this.carRepo.findOne({ where: { id: updateAdopdetCarDto.car_id } })
      if (!car) {
        throw new NotFoundException('Car not found')
      }

      // shu car id boshqa joyda bandmi yoki yoqmi tekshiradi
      const existingAdopdet = await this.adopdet_carRepo.findOne({ where: { car: { id: updateAdopdetCarDto.car_id } } });
  ...
  async removeAdopdet_car(id: string) {
    const adopdet = await this.adopdet_carRepo.findOne({ where: { id } })
    if (!adopdet) {
      throw new NotFoundException('Adopdet_Car not found')
    }

    await this.adopdet_carRepo.delete(id)
    return successRes({})
  }
}
